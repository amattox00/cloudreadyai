...
echo "==[ Resetting app/routers/workloads.py to known-good version ]=="
...
def list_workloads() -> WorkloadListResponse:
    ...
    This is forgiving: it reads raw JSON and extracts basic identifying fields
    without re-validating as a full Workload model, so older or slightly
    different schemas don't break this endpoint.
    """
    base = _ensure_base_dir()
    items: List[WorkloadSummary] = []
...
# 2) Quick sanity check in-process (optional)
python3 - << 'PY'
from app.routers.workloads import list_workloads
from pathlib import Path

print("Quick sanity check: list_workloads() returned:")
lst = list_workloads()
