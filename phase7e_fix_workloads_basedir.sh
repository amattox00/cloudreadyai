...
echo "==[ Fixing app/routers/workloads.py BASE_DIR to be backend-root-relative ]=="
...
echo "Rewrote $WORKLOAD_ROUTER with backend-root-relative BASE_DIR"

# 1) Compile check
python3 -m compileall "$WORKLOAD_ROUTER" >/dev/null
echo "✅ workloads router compile check passed."

# 2) In-process sanity check (reads backend/data/workloads)
python3 - << 'PY'
from app.routers.workloads import list_workloads

print("Sanity check: list_workloads() returned:")
lst = list_workloads()
