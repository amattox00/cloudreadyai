...
import RunIngestionTab from "../../components/RunIngestionTab";
...
export default function RunDetailPage() {
  ...
  return (
    <div className="px-6 py-6 space-y-6">
      ...
      {/* Tab content */}
      {activeTab === "overview" && (
        <OverviewTab assessment={assessment} ingestion={mockIngestionStats} />
      )}

      {activeTab === "ingestion" && (
        <div className="space-y-6">
          {/* Existing ingestion summary UI */}
          <IngestionTab stats={mockIngestionStats} />

          {/* NEW: slice-by-slice ingestion control with CSV upload */}
          <RunIngestionTab runId={assessment.id} />
        </div>
      )}

      {activeTab === "insights" && <PlaceholderTab title="Insights" />}
      {activeTab === "diagrams" && <PlaceholderTab title="Diagrams" />}
      {activeTab === "cost" && <PlaceholderTab title="Cost Modeling" />}
    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
      ...
      <div className="border border-gray-200 bg-white rounded-md shadow-sm p-4 space-y-3">
        ...
        <dl className="mt-2 grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-2 text-sm">
          <div>
            ...
            <dd className="text-gray-900">{assessment.client}</dd>
          </div>

          <div>
            <dt className="text-xs text-gray-500">Environment</dt>
            <dd className="text-gray-900">{assessment.environment}</dd>
          </div>

          <div>
            <dt className="text-xs text-gray-500">Status</dt>
            <dd className="text-gray-900">{assessment.status}</dd>
          </div>

          <div>
            <dt className="text-xs text-gray-500">Last updated</dt>
            <dd className="text-gray-900">{assessment.lastUpdated}</dd>
          </div>
        </dl>
      </div>
      ...
      <div className="border border-gray-200 bg-white rounded-md shadow-sm p-4 space-y-3">
        ...
        <dl className="mt-2 grid grid-cols-2 gap-x-4 gap-y-2 text-sm">
          <SummaryStat label="Servers" value={ingestion.servers} />
          <SummaryStat
            label="Storage volumes"
            value={ingestion.storageVolumes}
          />
          <SummaryStat label="Databases" value={ingestion.databases} />
          <SummaryStat label="Applications" value={ingestion.applications} />
          <SummaryStat
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
        <SummaryCard title="Servers ingested" value={String(stats.servers)} />
        <SummaryCard
          title="Storage volumes"
          value={String(stats.storageVolumes)}
        />
        <SummaryCard title="Databases" value={String(stats.databases)} />
        <SummaryCard title="Applications" value={String(stats.applications)} />
        <SummaryCard
...
function PlaceholderTab({ title }: { title: string }) {
  return (
    <div className="border border-gray-200 bg-white rounded-md shadow-sm p-4 text-sm text-gray-700">
      <h2 className="text-lg font-semibold mb-2 text-gray-900">{title}</h2>
      <p>
        This area will be wired to the {title.toLowerCase()} engine. For MVP,
        we&apos;ll start by surfacing key data and actions from the backend.
      </p>
    </div>
  );
}
...
function TableRow({
  ...
  return (
    <tr className={!last ? "border-b border-gray-100" : ""}>
      ...
      <td className="px-4 py-2 text-right text-gray-800">{count}</td>
